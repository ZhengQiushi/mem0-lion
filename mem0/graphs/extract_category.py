...
def get_default_profiles() -> str:
    return """basic_info: name, age, gender, location, contact
...
def pack_profiles_into_string(profiles):
    """Convert profiles dict to string format"""
    if not profiles or not profiles.get("facts"):
        return "No facts found"
    
    lines = []
    for fact in profiles["facts"]:
        lines.append(f"- {fact['topic']}\t{fact['sub_topic']}\t{fact['memo']}")
    return "\n".join(lines)


def get_prompt(topic_examples: str) -> str:
    """Get prompt for category extraction with segmented input format"""
    sys_prompt = os.getenv("MEM0_SYSTEM_PROMPT", DEFAULT_JOB)
    
    # Create examples with segmented format
    examples = "\n\n".join([
        f"""<example>
<input>
#### User Before topics
...
"""
    ...
    # Create a new prompt template for segmented input
    segmented_prompt = f"""{sys_prompt}
## Formatting
### Input
You will receive input in the following format:
#### User Before topics
[Existing user topics and subtopics, or "No existing categories found."]
Don't output the topics and subtopics that are not mentioned in the following conversation.
#### Memo
[The conversation or text to extract categories from]

### Output
#### Think
You need to think about what topics/subtopics are mentioned in the memo, or what implications can be inferred from the memo.
#### Profile
After your steps of thinking, you need to extract the facts and preferences from the memo and place them in order list:
- TOPIC{{tab}}SUB_TOPIC{{tab}}MEMO
For example:

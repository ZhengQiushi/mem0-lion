...
def test_category_graph():
    ...
    He is 9 years old and loves playing soccer. 
    His favorite color is blue and he likes pizza.
    """
    
    print("=== 测试Category子图功能 ===")
    ...
    try:
        ...
        print(f"\n添加结果:")
        print(f"- 删除的实体: {result.get('deleted_entities', [])}")
        print(f"- 添加的实体: {result.get('added_entities', [])}")
        print(f"- 添加的类别: {result.get('added_categories', [])}")
        ...
        if search_result1.get('category_relations'):
            print("类别子图结果:")
            for i, relation in enumerate(search_result1['category_relations'][:5]):
                print(f"  {i+1}. {relation}")
        else:
            print("  无类别子图结果")
        ...
        if search_result2.get('category_relations'):
            print("类别子图结果:")
            for i, relation in enumerate(search_result2['category_relations'][:5]):
                print(f"  {i+1}. {relation}")
        else:
            print("  无类别子图结果")
        ...
        if search_result3.get('category_relations'):
            print("类别子图结果:")
            for i, relation in enumerate(search_result3['category_relations'][:5]):
                print(f"  {i+1}. {relation}")
        else:
            print("  无类别子图结果")
